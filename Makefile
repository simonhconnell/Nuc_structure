...
#INCLUDES = -I$(ROOTSYS)/include -I/usr/local/Cellar/boost/1.71.0
...
#Uncomment this next line if you want to link to the ROOT FrameWork
#LDFLAGS = -L$(ROOTSYS)/lib `root-config --glibs` -L/usr/local/Cellar/boost/1.71.0 -lSpectrum
LDFLAGS = -L$(ROOTSYS)/lib `root-config --glibs`  -lSpectrum
